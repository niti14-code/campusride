 ...
 * Each college can have multiple accepted domains.
 */
...
/**
 * Returns the accepted domains for a given college name (case-insensitive partial match).
 * Returns [] if the college is not found.
 */
export function getDomainsForCollege(collegeName) {
...
/**
 * Validates whether an email's domain matches the college's allowed domains.
 * Returns { valid: boolean, message: string }
 */
export function validateCollegeEmail(email, collegeName) {
  if (!email || !collegeName) return { valid: false, message: '' };
  ...
  // College not in our list — require at least a non-generic domain
  if (allowedDomains.length === 0) {
    const genericDomains = ['gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'rediffmail.com'];
    if (genericDomains.includes(domain)) {
      return {
        valid: false,
        message: `Please use your official college email, not a personal one (${domain})`
      };
    }
    return { valid: true, message: '' }; // unknown college — allow non-generic
  }
  ...
  return {
    ...
    message: isValid
      ? ''
      : `Email must end with @${allowedDomains[0]} for ${collegeName} students`
  };
}
